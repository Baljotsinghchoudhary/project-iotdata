...
def home(request):
    ...
    predicted_motor_status=predicting(sensor_data['moisture'],sensor_data['temperature'],sensor_data['humidity'],10)
    return render(request,'home.html',{'sensor_data':sensor_data,'motor_data':motor_data,'weather_data':weather_data,'prediction':predicted_motor_status})
...
def getdata(request):
    ...
    weather_data=getweatherStatus()
    predicted_motor_status=predicting(sensor_data['moisture'],sensor_data['temperature'],sensor_data['humidity'],10)
    return JsonResponse({'sensor_data':sensor_data,'motor_data':motor_data,'weather_data':weather_data,'prediction':predicted_motor_status})
